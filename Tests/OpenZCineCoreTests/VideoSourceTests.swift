...
@Suite("Freed capture band")
struct FreedCaptureBandTests {
    private func availability(
        source: VideoSourceKind, owns: Bool, holdsControl: Bool
    ) -> MonitorDataAvailability {
        MonitorDataAvailability(
            source: source, ownsCameraSession: owns,
            receivesCameraMetadata: owns || source == .relay,
            holdsRelayControl: holdsControl)
    }

    @Test("A watcher without control owns the band")
    func plainWatcherFreesBand() {
        #expect(availability(source: .relay, owns: false, holdsControl: false).freesTheCaptureBand)
    }

    /// The regression Erik hit: taking control must release the band in the same breath, because
    /// the record control and capture strip arrive with the token.
    @Test("A watcher that takes control gives the band back immediately")
    func controllingWatcherKeepsOrdinaryLayout() {
        #expect(!availability(source: .relay, owns: false, holdsControl: true).freesTheCaptureBand)
    }

    /// The band must not stay free because a finished watcher session left the source kind behind:
    /// owning the camera decides it, whatever the source still says.
    @Test("Owning the camera decides it, even with a stale relay source")
    func staleRelaySourceCannotFreeTheBand() {
        #expect(!availability(source: .relay, owns: true, holdsControl: false).freesTheCaptureBand)
    }

    @Test("An operator never gives up the band, however the chrome is configured")
    func operatorKeepsBand() {
        #expect(
            !availability(source: .cameraLiveView, owns: true, holdsControl: false)
                .freesTheCaptureBand)
    }
}
