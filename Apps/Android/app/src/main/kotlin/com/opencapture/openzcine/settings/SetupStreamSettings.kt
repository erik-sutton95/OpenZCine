 ...
 *
 * A stored value is the operator's; `null` means they have never chosen one HERE, which is not the
 * same as choosing the default. Only the second case takes a default, and only the access point
 * gets one that differs.
 *
 * Twin of the shared core's `SetupStreamSettings`; both are tested against the same table so a path
 * can never mean one bias on iPhone and another on Android. Change one, change both.
 */
public object SetupStreamSettings {
    /**
     * The bias a setup runs at: the operator's own choice, or what this path should start at.
     *
     * The access point starts at [LiveViewQualityBias.LATENCY] — Fast. It is the one path whose
     * bandwidth is fixed by the camera's own radio rather than by the network someone built, and
     * it is where dropped frames and stalls actually get reported from. Every other path — a
     * router, a hotspot, a cable — starts from the seed, because nothing about them is known to
     * be narrow.
     */
    public fun qualityBias(
        stored: LiveViewQualityBias?,
        transport: SavedCameraTransport?,
        seed: LiveViewQualityBias,
    ): LiveViewQualityBias {
        if (stored != null) return stored
        return if (transport == SavedCameraTransport.CAMERA_ACCESS_POINT) {
     ...
     */
    public fun streamPreset(
        stored: LiveViewStreamPreset?,
        @Suppress("UNUSED_PARAMETER") transport: SavedCameraTransport?,
        seed: LiveViewStreamPreset,
    ): LiveViewStreamPreset = stored ?: seed
}
